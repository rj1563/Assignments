...
SELECT region_id, COUNT(node_id) AS no_of_nodes
FROM customer_nodes
GROUP BY region_id
ORDER BY region_id;
...
SELECT ROUND(AVG(end_date - start_date), 1) AS avg_number_of_days
FROM customer_nodes
WHERE end_date != '9999-12-31';

-- Query 5 - What is the median, 80th and 95th percentile for this same 
-- reallocation days metric for each region?
WITH CTE AS(
	SELECT customer_id, region_id, region_name, (end_date - start_date) AS reloc
	FROM customer_nodes
	JOIN regions USING(region_id)
	WHERE end_date != '9999-12-31'
)
SELECT region_id, region_name, PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY reloc) AS median,
...
-- Query 2 - What is the average total historical deposit counts and amounts
-- for all customers?
WITH CTE AS(
...
WITH CTE AS(
	...
	COUNT(
		CASE WHEN txn_type = 'deposit' THEN 1 END) AS deposit_count,
	COUNT(
		CASE WHEN txn_type = 'purchase' THEN 1 END) AS purchase_count,
	COUNT(
		CASE WHEN txn_type = 'withdrawal' THEN 1 END) AS withdrawal_count
	FROM customer_transactions
...
-- Query 4 - What is the closing balance for each customer at the end of 
-- the month?
WITH CTE AS(
		SELECT customer_id, DATE_PART('month', txn_date) AS months,
		SUM(
...
SELECT cte.customer_id, cte.months, SUM(cte.total_amount) OVER(PARTITION BY 
cte.customer_id ORDER BY cte.months) AS closing_balance
FROM CTE, customer_transactions
GROUP BY cte.customer_id, cte.months, cte.total_amount;

-- Query 5 What is the percentage of customers who increase their closing 
...
SELECT cte.customer_id, cte.months, SUM(cte.total_amount) AS closing_balance
FROM CTE
GROUP BY cte.customer_id, cte.months
ORDER BY cte.customer_id, cte.months;
